...
- 같은 `message-id`를 두 번 소비 → `restaurant_replicas` 1행, `processed_messages` 1행
- **두 번째 소비 후에도 `processed_at`이 첫 소비 시각 그대로인지** 확인한다. 행 수만 세면 부족하다 — 중복이 UPDATE로 처리되는 잘못된 구현도 1행을 유지하므로 테스트가 그냥 통과한다
- 다른 `message-id`면 2행
